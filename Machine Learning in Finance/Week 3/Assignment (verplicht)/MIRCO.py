...
class MIRCOfit:
    ...
    def exportRules(self):
        upperbounds_temp =  np.empty((500,500))
        upperbounds_temp[:] = np.nan
        lowerbounds_temp =  np.empty((500,500))
        lowerbounds_temp[:] = np.nan
        
        #lowerbounds = np.empty((10*len(list(self.rules.values())),1    ))
        #upperbounds = np.empty((10*len(list(self.rules.values())),1))
        lowerbounds = np.empty((10*len(list(self.rules.values())),len(list(self.rules.values()))))
        upperbounds = np.empty((10*len(list(self.rules.values())),len(list(self.rules.values()))))
        
        lowerbounds[:] = np.nan
        upperbounds[:] = np.nan
        
        saved1 = list(self.rules.values())
        saved2 = list(self.rules.values())
        saved3 = list(self.rules.values())
        saved4 = list(self.rules.values())
        contains_duplicates = list(self.rules.values())
        ...
        for rindx, rule in enumerate(self.rules.values()):
            ...
            for clause in rule[:-1]:
                ...
                saved3[m] = c
                saved4[m] = d
            #float in saved3[j] is not iterable
                contains_duplicates[m] = any([saved3[m]].count(element) > 1 for element in [saved3[m]])
            
                if (clause[1] == 'l'):
                    #print('==> x[%d] <= %.2f' % (clause[0], clause[2]))
                    upperbounds_temp[k,0] = clause[2]
                        
                if (clause[1] == 'r'):
                    #print('==> x[%d] > %.2f' % (clause[0], clause[2]))
                    lowerbounds_temp[k,0] = clause[2]           
                    
                if contains_duplicates[j]:
                    upperbounds[k,rindx] = np.nanmin(upperbounds_temp[k,:])
                    lowerbounds[k,rindx] = np.nanmax(lowerbounds_temp[k,:],axis=0)
                else:
                    upperbounds[k,rindx] = upperbounds_temp[k,0]
                    lowerbounds[k,rindx] = lowerbounds_temp[k,0]
                m=m+1
                k=k+1
